...
def get_oof(cfg):
    oof = np.load(f'../logs/{cfg.data.features.oof.name}/oof.npy')
    preds = np.load(f'../logs/{cfg.data.features.oof.name}/raw_preds.npy')

    if cfg.common.drop is not None:
        drop_idx = get_drop_idx(cfg.common.drop)
        oof = fill_dropped(oof, drop_idx)
